...
function SpChanged()
{
   if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete")
   {
           document.getElementById("StockTypeData").innerHTML= xmlHttp.responseText;
   }
   else {
           //alert(xmlHttp.status);
   }
...
// Will Stockpint Type data based on input
function StockTypeData(url, qStr)
{
   if (url.length==0)
   {
       document.getElementById("StockTypeData").innerHTML="";
       return;
   }
   xmlHttp=GetXmlHttpObject()
   if (xmlHttp==null)
   ...
   url=url+"?"+qStr;
   url=url+"&sid="+Math.random();
   xmlHttp.onreadystatechange=SpChanged;
   xmlHttp.open("GET",url,true) ;
   xmlHttp.send(null);
}
