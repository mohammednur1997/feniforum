...
@endsection
@section('title', translate('Edit_campaing'))
...
@endpush()
   ...
   <!-- Container fluid  -->
    ...
    <form action="{{ route('admin.event.update', $value->id) }}" method="post"  enctype="multipart/form-data">
                         
                      {{csrf_field()}}

                               <div class="form-group">
                                       <label for="validationCustom02">{{ translate('title')}}</label> <span class="text-danger">*</span>
                                       <input type="text" tabindex="1" name="title" class="form-control" value="{{ $value->title }}" required>
                              ...
                              <select class="form-control  custom-select" name="event_id"  style="width: 100%; height:36px;" required>
                                @foreach(App\EventName::get() as $event)
                                <option value="{{ $event->id }}" {{ $event->id == $value->event_id?"selected":"" }}>{{ $event->event_name }}</option>
                                @endforeach
                                    </select>
                                        <div class="invalid-feedback">
                              ...
                              <select class="form-control  custom-select" name="eventYear_id"  style="width: 100%; height:36px;" required>
                                ...
                                <option value="{{ $event->id }}" {{ $event->id == $value->  eventYear_id?"selected":"" }}>{{ $event->year_name }}</option>
                                @endforeach
                                    </select>
                                        <div class="invalid-feedback">
                                           {{translate('please_type_select')}}
                      ...
                      <label for="category">{{ translate('Event Category')}}</label> <span class="text-danger">*</span>
                        ...
                        <select class="form-control"  name="campCategeroy_id" required>
                          ...
                          <option value="{{ $category->id }}" {{ $category->id == $value->campCategeroy_id?"selected":"" }}>{{ $category->category_name }}</option>
                                @endforeach
                                ...
                                <div class="form-group">
                                      ...
                                      <input type="text"  name="campaing_location" class="form-control" value="{{ $value->campaing_location }}">
                                  ...
                                  <div class="form-group">
                                       ...
                                       <textarea name="campaing_description" id="ckeditor" cols="50" rows="15" class="ckeditor" >
                                         {{ $value->campaing_description }}
                                       </textarea>
                                 ...
                                 @php $i=1; @endphp
                                     @foreach($value->evetnIamge as $image)   
                                        @if($i > 0)  
 <img height="200px" width="200px" src="{{ asset('upload/images/event/'. $image->image) }}">          
                                        @endif    
                                      @php $i--; @endphp
                                     @endforeach 
                                 <!-- Start image teg -->
                                ...
                                <div class="form-group">
                                 ...
                                 <!-- end image teg -->
        @youtube($value->campaing_video, ['width'=>200, 'height'=>200,])
                                 <div class="form-group">
                               <label for="Video">{{ translate('Camping Video url')}}</label>
                                  ...
                                  <div class="form-group">
                                       ...
                                       <input type="text" name="goal_amount" class="form-control"  value="{{ $value->goal_amount }}"  required>
                                        <div class="invalid-feedback">
                                   ...
                                   <div class="form-group">
                                       ...
                                       <input type="text" tabindex="1" name="campaing_start_date" class="form-control" id="startDate" value="{{ $value->campaing_start_date }}"  data-date-format='yyyy-mm-dd'  required>
                                        <div class="invalid-feedback">
                                  ...
                                  <div class="form-group">
                                       ...
                                       <input type="text"  name="campaing_end_date" class="form-control" id="endDate" value="{{ $value->campaing_end_date }}"  data-date-format='yyyy-mm-dd' required>
                                        <div class="invalid-feedback">
                                           {{translate('please_date_select')}}
                                        </div>
                            ...
                            <input type="radio" id="goal_amount" value="goal_amount" name="end_type" {{ $value->end_type == 'goal_amount'?"checked":"" }} class="custom-control-input" required>
                                    <label class="custom-control-label" for="goal_amount">{{translate('goal_amount')}}</label>
                                    </div>

                                    <div class="custom-control custom-radio">
                                    <input type="radio" id="end_date" value="end_date" name="end_type" {{ $value->end_type == 'end_date'?"checked":"" }} class="custom-control-input" required>
                                 ...
                                 <div class="form-group">
                                  <div class="custom-control custom-checkbox">
                                    ...
                                    <input type="checkbox" class="custom-control-input" value="1" {{ $value->add_to_feature == 1?"checked":"" }} name="add_to_feature" id="customControlValidation1">
