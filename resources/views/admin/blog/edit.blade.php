...
@endpush()
   ...
   <!-- Container fluid  -->
    ...
    <form   action="{{route('admin.blog.update', $editblog->id)}}" method="post" enctype="multipart/form-data">
   
                      {{csrf_field()}}

                                <div class="form-group">
                                       <label for="blog_title">{{ translate('blog_title')}}</label> <span class="text-danger">*</span>
                                       <input type="text"  name="blog_title" class="form-control" value="{{ $editblog->blog_title }}" required>
                                        <div class="invalid-feedback">
                                           {{translate('please_add_title')}}
                                        </div>
                                  </div>
                                <img height="200px" width="200px" src="{{ asset('upload/images/blog/'. $editblog->fet_image) }}">
                                <div class="form-group">
                                       <label for="fet_image">{{ translate('fet_image')}}</label> <span class="text-danger">*</span>
                                       <input type="file" name="fet_image" class="form-control"  >
                                        <div class="invalid-feedback">
                                 ...
                                 <div class="form-group">
                                      ...
                                      <textarea name="blog_description" id="ckeditor" rows="5" cols="5" class="ckeditor">
                              ...
                              <option value="{{ $value->id }}" {{ $value->id == $editblog->cat_id?"selected":"" }}>{{$value->name}}</option>
